    ...
    public class PatientDispatchManager : IPatientDispatchManager
    {
        ...
        public async Task<string> createPatientDispatch(PatientDispatchEntity patientDispatch)
        {
            try
            {
                ...
                _logger.LogInformation("Created patient dispatch for patient id {patientDispatch.PatientId} in facility {patientDispatch.FacilityId}", HtmlInputSanitizer.Sanitize(patientDispatch.PatientId), HtmlInputSanitizer.Sanitize(patientDispatch.FacilityId));
                ...
                await ScheduleService.CreateJobAndTrigger(patientDispatch, await _schedulerFactory.GetScheduler());
                    ...
                    {
                        FacilityId = HtmlInputSanitizer.Sanitize(patientDispatch.FacilityId),
            ...
            {
                _logger.LogError(ex, "Create patient dispatch exception for patient id {HtmlInputSanitizer.Sanitize(patientDispatch.PatientId)} in facility {HtmlInputSanitizer.Sanitize(patientDispatch.FacilityId)}.", HtmlInputSanitizer.Sanitize(patientDispatch.PatientId), HtmlInputSanitizer.Sanitize(patientDispatch.FacilityId));
