...
namespace LantanaGroup.Link.QueryDispatch.Presentation.Controllers
{
    ...
    public class QueryDispatchController : ControllerBase
    {
        ...
        public async Task<ActionResult<RequestResponse>> CreateQueryDispatchConfigurationAsync(QueryDispatchConfiguration model, CancellationToken cancellationToken)
        {
            ...
            {
                _logger.LogError("Query dispatch configuration for Facility Id {FacilityId} was already created: {model}.", HtmlInputSanitizer.Sanitize(model.FacilityId), HtmlInputSanitizer.Sanitize(JsonSerializer.Serialize(model)));
                return BadRequest($"FacilityID {HtmlInputSanitizer.Sanitize(model.FacilityId)} configuration was already created.");
            }
            ...
            try
            {
                ...
                if (!facilityCheckResult)
                    return BadRequest($"Facility {HtmlInputSanitizer.Sanitize(model.FacilityId)} does not exist.");
        ...
        public async Task<ActionResult<RequestResponse>> UpdateQueryDispatchConfiguration(string facilityId, QueryDispatchConfiguration model, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (!facilityCheckResult)
                {
                    return BadRequest($"Facility {HtmlInputSanitizer.Sanitize(facilityId)} does not exist.");
                }
